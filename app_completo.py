...
# =============================================================================
# APLICAÇÃO COMPLETA v3: EXIBIÇÃO DE MÍDIA (API DIRETA DA META)
# =============================================================================
        ...
        try:
            if 'entry' in data and data['entry'][0]['changes'][0]['value'].get('messages'):
                ...
                nova_mensagem = {
                    ...
                    "telefone": remetente,
                    "texto": mensagem_para_painel,
                    "media_id": media_id,
                    "media_type": message_type if media_id else 'text'
                }
...
# NOVO: Endpoint para buscar e servir a mídia
@app.route('/media/<media_id>')
def get_media(media_id):
    if not META_ACCESS_TOKEN:
        return "Token de acesso não configurado", 500
    ...
    try:
        ...
        # 3. Servir a mídia para o navegador
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createMediaElement(msg) {
        if (!msg.media_id) return `<p class="mt-2 text-sm text-slate-700">${msg.texto}</p>`;
        if (msg.media_type === 'image') {
            return `<img src="/media/${msg.media_id}" class="w-full h-auto rounded mt-2 cursor-pointer" onclick="window.open('/media/${msg.media_id}', '_blank')">`;
        }
        return `<div class="mt-2"><a href="/media/${msg.media_id}" target="_blank" class="text-sm text-blue-600 hover:underline">Ver ${msg.media_type}</a></div>`;
    }
    ...
    function renderizarReclamacoes() {
        ...
        if (reclamacoes.length === 0) {
            complaintsList.innerHTML = '<p class="text-slate-400 text-center">Nenhuma reclamação registrada.</p>'; return;
        }
    ...
    function addPromoteListeners() {
        document.querySelectorAll('.promote-btn').forEach(btn => {
            btn.addEventListener('click', (event) => {
                promoverMensagem(parseInt(event.target.dataset.id));
            });
        });
    }
...
if __name__ == '__main__':
    print("===================================================")
    print("🚀 Servidor do Painel v3 (Modo Meta API) iniciado!")
