...
# =============================================================================
# APLICAÇÃO COMPLETA v2: PAINEL COM CAIXA DE ENTRADA (API DIRETA DA META)
# =============================================================================
...
# Busca as credenciais do ambiente do servidor (configuradas no Render)
...
db_reclamacoes = []
db_mensagens_recebidas = [] # NOVO: Armazena todas as mensagens
reclamacao_id_counter = 1
mensagem_id_counter = 1 # NOVO: Contador para as mensagens
...
def adicionar_ao_sorteio(telefone, nome_extraido):
    if telefone not in db_participantes_sorteio:
        ...
        return True # Retorna True se foi um novo participante
    return False
    ...
    if request.method == 'POST':
        ...
        try:
            if 'entry' in data and data['entry'][0]['changes'][0]['value'].get('messages'):
                ...
                mensagem_texto = message_data['text']['body']
                
                print(f"Mensagem recebida de {remetente}: '{mensagem_texto}'")
                nome_extraido = extrair_nome(mensagem_texto)

                # Adiciona a mensagem à caixa de entrada
                nova_mensagem = {
                    "id": mensagem_id_counter,
                    "nome": nome_extraido or f"Pessoa ({remetente[-4:]})",
                    "telefone": remetente,
                ...
                # Adiciona ao sorteio (apenas uma vez) e envia resposta
...
# NOVO endpoint para servir a caixa de entrada
@app.route('/mensagens', methods=['GET'])
def get_mensagens():
    return jsonify(db_mensagens_recebidas)

# NOVO endpoint para promover uma mensagem para reclamação
...
def promover_reclamacao():
    ...
    mensagem_a_promover = None
    for msg in db_mensagens_recebidas:
        if msg['id'] == mensagem_id:
            mensagem_a_promover = msg
            break
    
    if mensagem_a_promover:
        nova_reclamacao = {
            "id": reclamacao_id_counter,
            "nome": mensagem_a_promover['nome'],
            "telefone": mensagem_a_promover['telefone'],
            "texto": mensagem_a_promover['texto'],
            "status": "Registrada"
        }
        db_reclamacoes.append(nova_reclamacao)
        reclamacao_id_counter += 1
        
        # Remove a mensagem da caixa de entrada para não ser promovida duas vezes
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="bg-white p-6 rounded-xl shadow-lg lg:col-span-1"><h2 class="text-2xl font-bold text-center mb-4 border-b pb-3 text-red-600">Fala que Eu Registro</h2><div class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6"><h3 class="font-bold text-lg text-center mb-3">Placar da Comunidade</h3><div class="flex justify-around text-center"><div><p class="text-3xl font-bold text-slate-800" id="registered-count">0</p><p class="text-sm text-slate-500">Registradas</p></div><div><p class="text-3xl font-bold text-green-600" id="solved-count">0</p><p class="text-sm text-slate-500">Solucionadas</p></div></div></div><div id="complaints-list" class="space-y-3 max-h-96 overflow-y-auto pr-2"></div></div>
</div></div>
<script>
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchData() {
        try {
            const [participantesRes, reclamacoesRes, mensagensRes] = await Promise.all([
                fetch(`${API_URL}/participantes`), fetch(`${API_URL}/reclamacoes`), fetch(`${API_URL}/mensagens`)
            ]);
            participantesSorteio = await participantesRes.json();
            reclamacoes = await reclamacoesRes.json();
            mensagensRecebidas = await mensagensRes.json();
            renderizarTudo();
    ...
    function renderizarMensagens() {
        ...
        if (mensagensRecebidas.length === 0) {
            messagesList.innerHTML = '<p class="text-slate-400 text-center">Nenhuma nova mensagem.</p>';
            return;
        }
        ...
        mensagensRecebidas.forEach(msg => {
            ...
            card.innerHTML = `
                <div><p class="font-bold text-sm">${msg.nome}</p><p class="text-xs text-slate-500">${msg.telefone}</p></div>
                <p class="mt-2 text-sm text-slate-700">${msg.texto}</p>
                <button data-id="${msg.id}" class="promote-btn w-full text-xs bg-cyan-500 text-white font-semibold py-1 px-2 rounded hover:bg-cyan-600 transition mt-2">Promover para Reclamação</button>
            `;
    ...
    function renderizarParticipantes() {
        ...
        if (participantesSorteio.length === 0) {
            ...
            sorteioPlaceholder.textContent = 'Aguardando participantes...';
        } else {
            ...
            sorteioPlaceholder.textContent = 'Clique para iniciar o sorteio!';
        }
    }
    ...
    function renderizarReclamacoes() {
        ...
        if (reclamacoes.length === 0) {
            complaintsList.innerHTML = '<p class="text-slate-400 text-center">Nenhuma reclamação registrada.</p>';
            return;
        }
        reclamacoes.sort((a, b) => b.id - a.id);
    ...
    function realizarSorteio() {
        ...
        const participantElements = Array.from(participantsList.children);
        let rouletteInterval = setInterval(() => {
            ...
            participantElements[randomIndex].classList.add('highlight');
        }, 100);
        setTimeout(() => {
            ...
            participantElements.forEach(el => el.classList.remove('highlight'));
            participantElements[winnerIndex].classList.add('highlight');
...
if __name__ == '__main__':
    print("===================================================")
    print("🚀 Servidor do Painel v2 (Modo Meta API) iniciado!")
    print("Acesse o painel em: http://12.0.0.1:5000")
    print("===================================================")
